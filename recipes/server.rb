...
sysadmins = []
data_bag = node['munin']['data_bag']
if Chef::Config[:solo]
  sysadmins = data_bag(data_bag).map { |user| data_bag_item(data_bag, user) }
else
  sysadmins = search(data_bag, 'groups:sysadmin')
end
...
if Chef::Config[:solo]
  ...
  munin_servers = []
  if node['munin']['multi_environment_monitoring']
    if node['munin']['opsworks_enabled']
      munin_servers = search(:node, node['munin']['opsworks_layer'])
    elsif node['munin']['multi_environment_monitoring'].kind_of?(Array)
